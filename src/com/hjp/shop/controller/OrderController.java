...
import com.hjp.shop.Interceptor.LoginRequiredInterceptor;
...
public class OrderController extends Controller {
	...
	public void add(){
		if (getSessionAttr("user") == null) {
			setSessionAttr(LoginRequiredInterceptor.REDIRECT_AFTER_LOGIN, getRequest().getRequestURI());
			redirect("/user/login");
			return;
		}
	...
	public void list(){
		...
		if (getPara()!=null) {
			try {
				...
				pageNo = 1;
				org.apache.log4j.Logger.getLogger(OrderController.class).warn("list pageNo parse error", e);
			}
		}else {
			pageNo = 1;
		}
